...
const notificationPosition = ref<NotificationPosition>(
    readNotificationPosition(),
);
...
const projectListContainer = useTemplateRef<HTMLElement>(
    "projectListContainer",
);
const groupNameInput = useTemplateRef<HTMLInputElement>("groupNameInput");
const confirmationCancelButton = useTemplateRef<HTMLButtonElement>(
    "confirmationCancelButton",
);
const contextMenuEditButton = useTemplateRef<HTMLButtonElement>(
...
const runListScrollbar = ref<ScrollbarState>({
    overflow: false,
    hovered: false,
    thumbHeight: 0,
    thumbTop: 0,
});
const logScrollbar = ref<ScrollbarState>({
    overflow: false,
    hovered: false,
    thumbHeight: 0,
    thumbTop: 0,
});
const projectListScrollbar = ref<ScrollbarState>({
    overflow: false,
    hovered: false,
    thumbHeight: 0,
    thumbTop: 0,
...
function readTheme(): Theme {
    try {
        return window.localStorage.getItem("runvoke-theme") === "dark"
            ? "dark"
            : "light";
    } catch {
        return "light";
...
function readUngroupedCollapsed() {
    try {
        return (
            window.localStorage.getItem("runvoke-ungrouped-collapsed") ===
            "true"
        );
    } catch {
        return false;
...
function readLogLinkAction(): LogLinkAction {
    try {
        return window.localStorage.getItem("runvoke-log-link-action") === "copy"
            ? "copy"
            : "open";
    } catch {
        return "open";
...
function readGithubLinkVisible() {
    try {
        return (
            window.localStorage.getItem("runvoke-github-link-visible") !==
            "false"
        );
    } catch {
        return true;
...
function readPreviewUpdatesEnabled() {
    try {
        return (
            window.localStorage.getItem("runvoke-preview-updates-enabled") ===
            "true"
        );
    } catch {
        return false;
...
function readNotificationPosition(): NotificationPosition {
    ...
    try {
        const value = window.localStorage.getItem(
            "runvoke-notification-position",
        ) as NotificationPosition | null;
    ...
    try {
        return (
            window.localStorage.getItem(
                "runvoke-notification-stacking-enabled",
            ) === "true"
        );
    } catch {
        return false;
...
watch(
    ...
    (value) => {
        ...
        if (isTauri()) {
            const backgroundColor: [number, number, number] = dark
                ? [23, 26, 24]
                : [247, 245, 241];
...
watch(previewUpdatesEnabled, (enabled) => {
    try {
        window.localStorage.setItem(
            "runvoke-preview-updates-enabled",
            String(enabled),
        );
    } catch {
...
function isLocalhostUrl(url: string) {
    try {
        ...
        return (
            ...
            parsedUrl.hostname.toLocaleLowerCase() === "localhost"
        );
    } catch {
        return false;
    }
...
async function openRepository() {
    try {
        await invoke("open_external_url", {
            url: "https://github.com/GezelligheidLin/Runvoke",
        });
    } catch (value) {
...
function copyTextFallback(text: string) {
    ...
    textarea.remove();
    if (!copied) throw new Error("系统拒绝了剪贴板操作");
}
...
const filteredProjects = computed(() => {
    ...
    return projects.value.filter((project) =>
        `${project.name} ${project.directory} ${project.command}`
            .toLocaleLowerCase()
            .includes(keyword),
    );
});
...
const projectGroupSections = computed<ProjectGroupSection[]>(() => {
    ...
    const sections: ProjectGroupSection[] = projectGroups.value.map(
        (group) => ({
            id: group.id,
            name: group.name,
            collapsed: searching ? false : group.collapsed,
            projects: filteredProjects.value.filter(
                (project) => project.groupId === group.id,
            ),
        }),
    );
    const ungroupedProjects = filteredProjects.value.filter(
        (project) => !project.groupId,
    );
    if (
        ungroupedProjects.length ||
        (!searching && projectGroups.value.length)
    ) {
        sections.push({
            id: null,
            name: "未分组",
    ...
    return searching
        ? sections.filter((section) => section.projects.length)
        : sections;
});
...
const projectGroupCollapseAllLabel = computed(() =>
    allProjectGroupsCollapsed.value ? "展开全部分组" : "收起全部分组",
);

const visibleLogs = computed(() => {
    if (logFilter.value === "all") return selectedLogs.value;
    return selectedLogs.value.filter(
        (entry) => entry.stream === logFilter.value,
    );
});
...
const primaryLocalhostLogEntryId = computed(() => {
    ...
    for (const entry of selectedLogs.value) {
        const containsLocalhostUrl = splitLogMessage(entry.message).some(
            (segment) => segment.url && isLocalhostUrl(segment.url),
        );
        if (!containsLocalhostUrl) continue;
...
async function locatePrimaryLocalhostLink() {
    ...
    const targetRow = Array.from(
        container?.querySelectorAll<HTMLElement>(".log-line") ?? [],
    ).find((row) => row.dataset.logEntryId === entryId);
    const targetLink = Array.from(
        targetRow?.querySelectorAll<HTMLButtonElement>(".log-link") ?? [],
    ).find((link) => link.dataset.localhostLink === "true");
    if (!container || !targetLink) return;
...
const activeRuns = computed(() =>
    Object.values(runsById.value).filter((run) => isRunActive(run.state)),
);
const runningCount = computed(() => activeRuns.value.length);
const operationsLocked = computed(
    () => updateInstalling.value || busyAction.value === "stop-all",
);
const visibleTasks = computed(
    () => selectedProject.value?.tasks.slice(0, 3) ?? [],
);
const inactiveRunCount = computed(
    () => projectRuns.value.filter((run) => !isRunActive(run.state)).length,
);
...
watch(
    () =>
        projectGroupSections.value
            .map(
                (section) =>
                    `${section.id}:${section.collapsed}:${section.projects.length}`,
            )
            .join("|"),
    () => {
...
watch(settingsOpen, (open) => {
    if (!open) void nextTick(updateScrollbars);
});
...
onBeforeUnmount(() => {
    ...
    if (mcpWorkspaceRefreshTimer) clearTimeout(mcpWorkspaceRefreshTimer);
    for (const unlisten of mcpUnlisteners) unlisten();
    mcpUnlisteners = [];
});
...
async function showRuntimeDesktopNotification(
    ...
    if (!isTauri() || !(await isAppInBackground())) return;
...
function projectNameForRun(run: RuntimeStatus) {
    return (
        projects.value.find((project) => project.id === run.projectId)?.name ??
        "项目"
    );
}

function markRunNotification(runId: string) {
    if (notifiedRunIds.has(runId)) return false;
    notifiedRunIds.add(runId);
    if (notifiedRunIds.size > 1_000) {
        const oldestRunId = notifiedRunIds.values().next().value;
        if (oldestRunId) notifiedRunIds.delete(oldestRunId);
    }
    return true;
}

function reportRuntimeStatusChange(
    run: RuntimeStatus,
    ...
    if (!hasEnded || !markRunNotification(run.runId)) return;
...
async function setupMcpBridge() {
    ...
    try {
        mcpUnlisteners = await Promise.all([
            ...
            listen<{ projects?: unknown }>(
                "mcp-project-import-request",
                ({ payload }) => {
                    if (
                        !Array.isArray(payload.projects) ||
                        !payload.projects.every(isAgentImportCandidate)
                    ) {
            ...
            listen<PreviewUpdateDownloadProgress>(
        ...
        mcpStatus.value = await invoke<McpServerStatus>(
...
async function setMcpServerEnabled(enabled: boolean) {
    ...
    try {
        ...
        notify(
            "success",
            enabled ? "本地 MCP 服务已开启" : "本地 MCP 服务已关闭",
        );
    } catch (value) {
    ...
    return JSON.stringify(
        {
            mcpServers: {
                runvoke: {
                    ...
                    url: status.endpoint,
                    headers: {
                        Authorization: `Bearer ${status.authorizationToken}`,
                    },
                },
            },
        },
...
async function showTestNotification() {
    if (!isTauri() || notificationTesting.value) return;
...
function compareVersions(left: string, right: string) {
    ...
    for (
        let index = 0;
        index < Math.max(leftParts.core.length, rightParts.core.length);
        index += 1
    ) {
        const difference =
    ...
    if (!leftParts.prerelease.length || !rightParts.prerelease.length)
        return leftParts.prerelease.length
            ? -1
    ...
    for (
        let index = 0;
        index <
        Math.max(leftParts.prerelease.length, rightParts.prerelease.length);
        index += 1
    ) {
...
async function checkForUpdate(showResult = false) {
    if (!isTauri() || updateInstalling.value || updateChecking.value) return;
        ...
        if (previewUpdatesEnabled.value) {
            try {
                preview = previewUpdate(
                    await invoke<PreviewUpdate | null>("check_preview_update"),
                );
            } catch (value) {
                if (!stable) throw value;
            }
        }
        availableUpdate.value =
            preview &&
            (!stable || compareVersions(preview.version, stable.version) > 0)
                ? preview
                : stable;
        if (availableUpdate.value && showResult) {
            notify(
                "success",
                `${availableUpdate.value.channel === "preview" ? "发现预览版本" : "发现新版本"} v${availableUpdate.value.version}`,
            );
        } else if (!availableUpdate.value && showResult) {
            notify("success", "当前已是最新版本");
        }
    } catch (value) {
        if (showResult) notify("error", `检查更新失败：${String(value)}`);
    } finally {
...
async function installAvailableUpdate() {
    ...
    try {
        ...
        else await update.update.downloadAndInstall(trackUpdateDownload);
        await relaunch();
    } catch (value) {
...
function promptProjectImportAfterUpdate(version: string) {
    try {
        ...
        if (!promptState) {
            window.localStorage.setItem(
                promptStateStorageKey,
                previousVersion ? "shown" : "pending",
            );
            return;
        }
...
async function loadProjectImportCandidates() {
    if (!isTauri() || projectImportLoading.value) return;

    projectImportLoading.value = true;
    try {
        const command =
            projectImportSource.value === "cursor"
                ? "list_cursor_projects"
                : "list_vscode_projects";
        const candidates = await invoke<ImportedProject[]>(command);
        const existingDirectories = new Set(
            projects.value.map((project) =>
                normalizeDirectory(project.directory),
            ),
        );
        projectImportCandidates.value = candidates.filter(
            (project) =>
                !existingDirectories.has(normalizeDirectory(project.directory)),
        );
    } catch (value) {
...
function openAgentProjectImportDialog(candidates: ImportedProject[]) {
    const existingDirectories = new Set(
        projects.value.map((project) => normalizeDirectory(project.directory)),
    );
    agentProjectImportCandidates.value = candidates.filter(
        (project) =>
            !existingDirectories.has(normalizeDirectory(project.directory)),
    );
...
function isAgentImportCandidate(
    candidate: unknown,
): candidate is ImportedProject {
    ...
    return (
        ...
        (value.suggestedCommand === null ||
            value.suggestedCommand === undefined ||
            typeof value.suggestedCommand === "string")
    );
}

async function importProjects(candidates: ImportedProject[]) {
    if (projectImporting.value || !candidates.length) return;
    ...
    try {
        ...
        for (const candidate of candidates) {
            if (
                existingDirectories.has(normalizeDirectory(candidate.directory))
            )
                continue;
            ...
            const project: ProjectConfig = {
                ...
                port: null,
                tasks: [
                    { id: "", name: "开发服务器", command, mode: "service" },
                ],
            };
            try {
        ...
        if (imported) {
            ...
            notify(
                "success",
                failed
                    ? `已导入 ${imported} 个项目，${failed} 个项目未能导入`
                    : `已导入 ${imported} 个项目`,
            );
        } else {
            notify(
...
async function importAgentProjects(candidates: ImportedProject[]) {
    if (agentProjectImporting.value || !candidates.length) return;
    ...
    try {
        ...
        if (imported) {
            ...
            notify(
                "success",
                failed
                    ? `已按确认纳入 ${imported} 个项目，${failed} 个项目未能纳入`
                    : `已按确认纳入 ${imported} 个项目`,
            );
        } else {
            notify(
...
async function openProjectConfigDirectory() {
    if (!isTauri() || projectConfigOpening.value) return;
...
function showProjectContextMenu(event: MouseEvent, project: ProjectConfig) {
    ...
    const left = Math.max(
        viewportPadding,
        Math.min(anchorX, window.innerWidth - menuWidth - viewportPadding),
    );
    const top = Math.max(
        viewportPadding,
        Math.min(anchorY, window.innerHeight - menuHeight - viewportPadding),
    );
    const submenuLeft =
        left + menuWidth + submenuWidth <= window.innerWidth - viewportPadding
            ? left + menuWidth - 1
            : left - submenuWidth + 1;
    projectContextMenu.value = {
        ...
        submenuTop: Math.max(
            viewportPadding,
            Math.min(top, window.innerHeight - submenuHeight - viewportPadding),
        ),
    };
...
async function handleSaveGroup() {
    ...
    try {
        ...
        const existing = projectGroups.value.find(
            (group) => group.id === groupDraftId.value,
        );
        const saved = await saveProjectGroup({
            id: groupDraftId.value,
            name,
        ...
        if (projectId) {
            try {
                const targetIndex = projects.value.filter(
                    (project) =>
                        project.groupId === saved.id &&
...
function requestInstallAvailableUpdate(event: MouseEvent) {
    ...
    const previewNotice =
        availableUpdate.value?.channel === "preview"
...
async function moveContextProjectToGroup(groupId: string | null) {
    ...
    const targetIndex = projects.value.filter(
        (item) => item.id !== project.id && item.groupId === groupId,
    ).length;
    const groupName = groupId
        ? projectGroups.value.find((group) => group.id === groupId)?.name
        : "未分组";
    await perform(
...
async function handleProjectMove(payload: {
    projectId: string;
    groupId: string | null;
    targetIndex: number;
}) {
    ...
    const targetProjects = projects.value.filter(
        (project) =>
            project.id !== payload.projectId &&
            project.groupId === payload.groupId,
    );
    const visibleTargets = targetProjects.filter((project) =>
    ...
    const targetIndex = nextVisibleProject
        ? targetProjects.findIndex(
              (project) => project.id === nextVisibleProject.id,
          )
        : previousVisibleProject
          ? targetProjects.findIndex(
                (project) => project.id === previousVisibleProject.id,
            ) + 1
...
function updateScrollbar(
    ...
    const thumbHeight = overflow
        ? Math.min(
              ...
              Math.max(
                  28,
                  Math.round(
                      (visibleHeight * visibleHeight) / scrollableHeight,
                  ),
              ),
          )
        : 0;
    ...
    if (scrollbar.overflow !== overflow) scrollbar.overflow = overflow;
    if (scrollbar.thumbHeight !== thumbHeight)
        scrollbar.thumbHeight = thumbHeight;
    if (scrollbar.thumbTop !== thumbTop) scrollbar.thumbTop = thumbTop;
}
...
function scheduleScrollbarUpdate() {
    if (scrollbarUpdateFrame !== undefined) return;
...
function setScrollbarHover(scrollbar: ScrollbarState, hovered: boolean) {
    ...
    if (hovered) void nextTick(updateScrollbars);
}

function startScrollbarDrag(
    kind: "projects" | "runs" | "logs",
    event: PointerEvent,
) {
    const container =
        kind === "projects"
            ...
            : kind === "runs"
              ? runListContainer.value
              : logContainer.value;
    const scrollbar =
        kind === "projects"
            ? projectListScrollbar.value
            : kind === "runs"
              ? runListScrollbar.value
              : logScrollbar.value;
    if (!container || !scrollbar.overflow) return;
    ...
    if (trackRange > 0 && scrollRange > 0)
        container.scrollTop = Math.min(
            ...
            Math.max(
                0,
                startScrollTop +
                    ((event.clientY - startY) * scrollRange) / trackRange,
            ),
        );
...
async function stopAllActiveRuns() {
    ...
    const runIds = runs
        .filter((run) => run.state !== "stopping")
        .map((run) => run.runId);
    const results = await Promise.allSettled(
        runIds.map((runId) => stopRunWithoutDesktopNotification(runId)),
    );
...
async function handleStopAllActiveRuns() {
    ...
    try {
        ...
        if (busyAction.value === "stop-all") busyAction.value = "";
    }
}
...
function projectState(projectId: string) {
    return (
        Object.values(runsById.value).find(
            (run) => run.projectId === projectId && isRunActive(run.state),
        )?.state ?? "stopped"
    );
}
...
function confirmStopRun(event: MouseEvent, run: RuntimeStatus) {
    requestConfirmation(
        ...
        () =>
            perform(
                `stop-${run.runId}`,
                () => stopRunWithoutDesktopNotification(run.runId),
                "任务已停止",
            ),
    );
}
...
async function handleTaskAction(event: MouseEvent, task: ProjectTask) {
    ...
    await perform(
        `task-${task.id}`,
        () => runTask(selectedProject.value!.id, task),
        `已开始执行「${task.name}」`,
    );
}
...
async function handleTemporaryCommand() {
    ...
    if (!project || !command || operationsLocked.value) return;
    await perform(
        "temporary",
        () => runTemporaryCommand(project.id, command),
        "临时命令已开始执行",
    );
    temporaryCommand.value = "";
}

function handleSaveOpenSource(source: OpenSourceConfig) {
    void perform(
        "save-open-source",
        () => saveOpenSource(source),
        "软件源已保存",
    );
}

function handleDeleteOpenSource(sourceId: string) {
    void perform(
        "delete-open-source",
        () => deleteOpenSource(sourceId),
        "软件源已删除",
    );
}

async function handleProjectOpen(action: "file-manager" | string) {
    projectOpenMenuOpen.value = false;
    const project = selectedProject.value;
    if (!project) return;
    if (action === "file-manager") {
        await perform(
            "open-file-manager",
            () => openInFileManager(project.directory),
            "已在文件管理器中打开",
        );
        return;
    }
    const source = openSources.value.find((item) => item.id === action);
    if (source)
        await perform(
            `open-source-${source.id}`,
            () => openWithSource(source.id, project.directory),
            `已交给 ${source.name} 打开`,
        );
}
                ...
                <template #start>
                    <aside class="sidebar">
                                ...
                                >
                                    <OverflowTooltip
                                        :text="
                                            search.trim()
                                                ? '搜索时保持分组展开'
                                                : projectGroupCollapseAllLabel
                                        "
                                        always
                                        >我的项目</OverflowTooltip
                                    >
                                </button>
                        ...
                        <label class="search-box">
                            ...
                            <input
                                v-model="search"
                                placeholder="搜索项目、目录或命令"
                            />
                        </label>

                        <div
                            class="scroll-fade-wrap project-list-scroll-wrap"
                            @mouseenter="
                                setScrollbarHover(projectListScrollbar, true)
                            "
                            @mouseleave="
                                setScrollbarHover(projectListScrollbar, false)
                            "
                        >
                            <nav
                                ...
                                class="project-list"
                                aria-label="项目列表"
                                @scroll="updateProjectListScrollbar"
                            >
                                <section
                                    v-for="section in projectGroupSections"
                                    :key="section.id ?? 'ungrouped'"
                                    class="project-group-section"
                                >
                                    <header
                                        v-if="
                                            section.id || projectGroups.length
                                        "
                                    ...
                                    <div
                                        ...
                                        @transitionend="
                                            handleProjectGroupTransitionEnd
                                        "
                                    >
                                        <div
                                            class="project-group-collapse-inner"
                                        >
                                            <ProjectGroupList
                                ...
                                <div
                                    v-if="
                                        !loading && !projectGroupSections.length
                                    "
                                    class="list-empty"
                                >
                                    <p>
                            ...
                            <span
                                v-if="projectListScrollbar.overflow"
                                class="scrollbar-rail light-scrollbar"
                                :class="{
                                    visible: projectListScrollbar.hovered,
                                }"
                            >
                                <i
                                    :style="{
                                        height: `${projectListScrollbar.thumbHeight}px`,
                                        transform: `translateY(${projectListScrollbar.thumbTop}px)`,
                                    }"
                                    @pointerdown="
                                        startScrollbarDrag('projects', $event)
                                    "
                        ...
                        <footer class="sidebar-footer">
                            ...
                            <span v-else-if="appVersion" class="version-label"
                                >v{{ appVersion }}</span
                            >
                            <Transition name="update-popover">
                                <section
                                    v-if="updatePopoverOpen && availableUpdate"
                                    class="update-popover"
                                    @keydown.esc="updatePopoverOpen = false"
                                >
                                    <span>{{
                                        availableUpdate.channel === "preview"
                                            ? "发现预览版本"
                                            : "发现新版本"
                                    }}</span>
                                    <strong
                                        >v{{ availableUpdate.version }}</strong
                                    >
                                    <small
                                        v-if="
                                            ...
                                            'preview'
                                        "
                                        class="preview-update-notice"
                                        >预览版本可能包含未完成或不稳定的功能。</small
                                    >
                                    <OverflowTooltip
                                        as="p"
                                        :text="
                ...
                <template #end>
                    <main class="workspace">
                        <SettingsPage
                            ...
                            :project-import-busy="
                                projectImportLoading || projectImporting
                            "
                            ...
                            @set-github-link-visible="
                                githubLinkVisible = $event
                            "
                            @set-preview-updates-enabled="
                                previewUpdatesEnabled = $event
                            "
                            @check-update="checkForUpdate(true)"
                            @install-update="requestInstallAvailableUpdate"
                            @open-project-config-directory="
                                openProjectConfigDirectory
                            "
                            @set-project-import-source="
                                projectImportSource = $event
                            "
                            @open-project-import="openProjectImportDialog"
                            @save-open-source="handleSaveOpenSource"
                            @delete-open-source="handleDeleteOpenSource"
                            @set-mcp-enabled="setMcpServerEnabled"
                            @copy-mcp-config="copyMcpConfig"
                            @set-notification-position="
                                notificationPosition = $event
                            "
                            @set-notification-stacking-enabled="
                                notificationStackingEnabled = $event
                            "
                        ...
                        <template v-else-if="selectedProject">
                            <header class="workspace-header">
                                <div class="project-title">
                                    ...
                                    <div class="project-heading">
                                        ...
                                        >
                                            ...
                                            >
                                                <path
                                                    d="M4 2.5v8.2a2.3 2.3 0 1 0 1.5 2.15V6.4h4.2a2.3 2.3 0 1 1-1.5 2.15"
                                                />
                                                <circle
                                                    cx="4"
                                                    cy="2.5"
                                            ...
                                            <span class="project-branch-label"
                                                >分支</span
                                            >
                                            <OverflowTooltip
                                                as="span"
                                                :text="`Git 分支：${selectedProject.gitBranch}`"
                                                >{{
                                            ...
                                            <span
                                                v-if="
                                                    selectedProject.stagedChanges !==
                                                        null &&
                                                    selectedProject.stagedChanges !==
                                                        undefined &&
                                                    selectedProject.unstagedChanges !==
                                                        null &&
                                                    selectedProject.unstagedChanges !==
                                                        undefined
                                                "
                                    ...
                                    <OverflowTooltip
                                        as="code"
                                        :text="selectedProject.directory"
                                        >{{
                                ...
                                <div class="header-actions">
                                    <span
                                        class="project-open-control"
                                        :class="{ open: projectOpenMenuOpen }"
                                        @focusout="closeProjectOpenMenu"
                                        @keydown.esc="
                                            projectOpenMenuOpen = false
                                        "
                                    >
                                        <button
                                            class="project-open-trigger"
                                            type="button"
                                            aria-haspopup="menu"
                                    ...
                                    <button type="button" @click="openEditForm">
                                        编辑
                                    </button>
                                    <button
                                        class="danger-link"
                            ...
                            <div class="project-workspace">
                                <section class="task-deck">
                                        ...
                                        <button
                                            ...
                                            :disabled="
                                                ...
                                                Boolean(
                                                    activeRunForTask(task.id) &&
                                                    busyAction ===
                                                        `stop-${activeRunForTask(task.id)!.runId}`,
                                                )
                                            "
                                            @click="
                                                handleTaskAction($event, task)
                                            "
                                        >
                                    ...
                                    <form
                                        class="temporary-command"
                                        @submit.prevent="handleTemporaryCommand"
                                    >
                                        <span>$</span>
                                        <input
                                            v-model="temporaryCommand"
                                            aria-label="临时命令"
                                            placeholder="输入一次性临时命令，例如 pnpm build"
                                        />
                                        <button
                                            type="submit"
                                            :disabled="
                                ...
                                <section class="run-workbench">
                                    <aside class="run-list-panel">
                                        <header>
                                            ...
                                            <div class="run-list-actions">
                                                <button
                                                    v-if="inactiveRunCount"
                                                    class="run-clear-finished"
                                                    type="button"
                                                    :disabled="
                                                        busyAction ===
                                                        'dismiss-inactive'
                                        ...
                                        <div
                                            class="scroll-fade-wrap run-list-scroll-wrap"
                                            @mouseenter="
                                                setScrollbarHover(
                                                    runListScrollbar,
                                                    true,
                                        ...
                                        >
                                            <div
                                                ref="runListContainer"
                                                class="run-list"
                                                @scroll="updateRunListScrollbar"
                                            >
                                                <div
                                                ...
                                                <p v-if="!projectRuns.length">
                                                    运行任务后会显示在这里
                                                </p>
                                            </div>
                                            <span
                                                v-if="runListScrollbar.overflow"
                                                class="scrollbar-rail light-scrollbar"
                                                :class="{
                                                    visible:
                                                        runListScrollbar.hovered,
                                                }"
                                            >
                                                <i
                                                    :style="{
                                                        height: `${runListScrollbar.thumbHeight}px`,
                                                        transform: `translateY(${runListScrollbar.thumbTop}px)`,
                                                    }"
                                                    @pointerdown="
                                                        startScrollbarDrag(
                                                            'runs',
                                    ...
                                    <section class="terminal-panel">
                                        <header class="terminal-toolbar">
                                            ...
                                            >
                                                ...
                                                <div class="terminal-actions">
                                                    <button
                                                        v-if="
                                                            primaryLocalhostLogEntryId
                                                        "
                                                        class="locate-link-button terminal-control-button"
                                                        type="button"
                                                        aria-label="定位本地地址"
                                                        title="定位本地地址"
                                                        @click="
                                                            locatePrimaryLocalhostLink
                                                        "
                                                    >
                                                        <svg
                                                            class="terminal-control-icon"
                                                            viewBox="0 0 24 24"
                                                            aria-hidden="true"
                                                        >
                                                            <circle
                                                    ...
                                                    <button
                                                        ...
                                                        "
                                                        class="clear-button terminal-control-button"
                                                        type="button"
                                                        aria-label="停止任务"
                                                        title="停止任务"
                                                        @click="
                                        ...
                                        <div
                                            class="scroll-fade-wrap terminal-scroll-wrap"
                                            @mouseenter="
                                                setScrollbarHover(
                                                    logScrollbar,
                                                    true,
                                        ...
                                        >
                                            ...
                                            <span
                                                v-if="logScrollbar.overflow"
                                                class="scrollbar-rail terminal-scrollbar"
                                                :class="{
                                                    visible:
                                                        logScrollbar.hovered,
                                                }"
                                            >
                                                <i
                                                    :style="{
                                                        height: `${logScrollbar.thumbHeight}px`,
                                                        transform: `translateY(${logScrollbar.thumbTop}px)`,
                                                    }"
                                                    @pointerdown="
                                                        startScrollbarDrag(
                                                            'logs',
                        ...
                        <section v-else class="workspace-empty">
                            <div class="empty-copy">
                                <span class="empty-eyebrow"
                                    ><i aria-hidden="true" /> 工作区 / 00</span
                                >
                                <h1>让第一个项目<br /><em>开始运行。</em></h1>
                                <p>
                                    从一个本地目录开始，把启动命令、运行状态和日志收进同一个工作台。
                                </p>
                                <button
                                ...
                                <div class="empty-queue">
                                    ...
                                    <div>
                                        ...
                                        ><OverflowTooltip
                                            as="b"
                                            text="进入后台运行"
                                            >进入后台运行</OverflowTooltip
                                        ><i>待命</i>
                                    </div>
                                </div>
                                <div class="empty-console-footer">
                                    <span>QUEUE</span><b>0 / 3</b>
                                </div>
                            </div>
                        </section>
                    </main>
            ...
            <Teleport to="body">
                <Transition name="project-context-menu">
                    <div
                        ...
                        class="project-context-layer"
                        @keydown.esc="closeProjectContextMenu"
                        @mousedown.self="closeProjectContextMenu"
                        @contextmenu.self.prevent="closeProjectContextMenu"
                    >
                        <section
                            ...
                            aria-label="项目操作"
                            :style="{
                                top: `${projectContextMenu.top}px`,
                                left: `${projectContextMenu.left}px`,
                            }"
                        >
                            <button
                                ref="contextMenuEditButton"
                                type="button"
                                role="menuitem"
                                @click="editProjectFromContextMenu"
                            >
                                编辑
                            </button>
                            <div
                                ...
                                :class="{
                                    'is-open': projectContextSubmenuOpen,
                                }"
                            ...
                            >
                                ...
                                <section
                                    ...
                                    aria-label="移动到分组"
                                    :style="{
                                        top: `${projectContextMenu.submenuTop}px`,
                                        left: `${projectContextMenu.submenuLeft}px`,
                                    }"
                                >
                                    <button
                                        type="button"
                                        role="menuitem"
                                        @click="createGroupFromContextMenu"
                                    >
                                        新建组
                                    </button>
                                    ...
                                    <button
                                        ...
                                        "
                                        @click="
                                            moveContextProjectToGroup(group.id)
                                        "
                                    >
                                        <i
                                            :class="{
                                    ...
                                    <button
                                        ...
                                        role="menuitemradio"
                                        :aria-checked="
                                            !projectContextMenu.project.groupId
                                        "
            ...
            <Teleport to="body">
                <Transition name="confirmation">
                    <div
                        ...
                        @keydown.esc="pendingConfirmation = null"
                        @mousedown.self="pendingConfirmation = null"
                    >
                        <section
                            ...
                            :style="{
                                top: `${pendingConfirmation.top}px`,
                                left: `${pendingConfirmation.left}px`,
                                width: `${pendingConfirmation.width}px`,
                            }"
                        >
                            ...
                            <div class="confirmation-actions">
                                ...
                                <button
                                    :class="
                                        pendingConfirmation.variant === 'danger'
                                            ? 'confirmation-danger'
