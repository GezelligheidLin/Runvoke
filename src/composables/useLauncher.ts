...
export function useLauncher(options: LauncherOptions = {}) {
  ...
  const openSources = ref<OpenSourceConfig[]>([]);
  ...
  const selectedRun = computed(
    () =>
      projectRuns.value.find((run) => run.runId === selectedRunId.value) ??
      projectRuns.value[0] ??
      null,
  );
  ...
  function appendLogs(payloads: IncomingLog[]) {
    for (const payload of payloads) {
      if (payload.projectId === "app") continue;
  ...
  function flushPendingLogs() {
    ...
    if (!pendingLogsByRunId.size) return;
    ...
    for (const [runId, pending] of pendingLogsByRunId) {
      ...
      for (const incoming of pending) {
        ...
        const effectiveMode: LogMode =
          mode === "append" && isReplaceableProgress(message)
            ? "progress"
            : mode;
        ...
        if (effectiveMode !== "append") {
          for (let index = entries.length - 1; index >= 0; index--) {
            if (
              entries[index]?.transient &&
              entries[index]?.stream === entry.stream
            ) {
              replaceIndex = index;
              break;
            }
        ...
        if (replaceIndex >= 0) entries[replaceIndex] = entry;
        else {
          if (effectiveMode === "append") {
            for (let index = entries.length - 1; index >= 0; index--) {
  ...
  async function refreshSelectedProjectGitStatus(projectId = selectedId.value) {
    ...
    const status = await invoke<ProjectGitStatus | null>(
  ...
  async function refreshOpenSources() {
    openSources.value = await invoke<OpenSourceConfig[]>("list_open_sources");
  }
  ...
  async function refreshRuntime(announce = false) {
    ...
    runsById.value = Object.fromEntries(
      statuses.map((status) => [status.runId, status]),
    );
    if (!announce) return;
  ...
  async function initialize() {
    ...
    try {
      unlisteners.push(
        await listen<IncomingLog[]>("project-logs", ({ payload }) =>
          appendLogs(payload),
        ),
        await listen<RuntimeStatus>("project-status", ({ payload }) =>
          updateRun(payload),
        ),
      );
      await Promise.all([
      ...
      pollTimer = setInterval(
        () => void refreshRuntime(true).catch(reportError),
        1_500,
      );
  ...
  async function reorderProjects(projectIds: string[]) {
    ...
    if (
      reordered.some((project) => !project) ||
  ...
  async function setProjectGroupCollapsed(groupId: string, collapsed: boolean) {
    ...
    projectGroups.value = previous.map((group) =>
      group.id === groupId ? { ...group, collapsed } : group,
    );
    try {
      await invoke<ProjectGroup>("set_project_group_collapsed", {
        groupId,
        collapsed,
      });
    } catch (value) {
  ...
  async function setProjectGroupsCollapsed(collapsed: boolean) {
    ...
    try {
      projectGroups.value = await invoke<ProjectGroup[]>(
  ...
  async function moveProject(
    ...
    if (sourceIndex < 0) throw new Error("项目不存在");
    const next = [...previous];
    ...
    const insertIndex =
      matchingIndices[targetIndex] ??
      (matchingIndices.length ? matchingIndices.at(-1)! + 1 : next.length);
    next.splice(insertIndex, 0, moved);
    projects.value = next;
    try {
      projects.value = await invoke<ProjectConfig[]>("move_project", {
        projectId,
        groupId,
        targetIndex,
      });
  ...
  async function removeProject(projectId: string) {
    ...
    const { [projectId]: removedGitStatus, ...remainingGitStatuses } =
      gitStatusByProjectId.value;
    ...
    logsByRunId.value = Object.fromEntries(
      Object.entries(logsByRunId.value).filter(
  ...
  async function runTask(projectId: string, task: ProjectTask) {
    const status = await invoke<RuntimeStatus>("run_task", {
      projectId,
      taskId: task.id,
    });
    updateRun(status);
  ...
  async function runTemporaryCommand(projectId: string, command: string) {
    const status = await invoke<RuntimeStatus>("run_temporary_command", {
      projectId,
      command,
    });
    updateRun(status);
  ...
  async function stopRun(runId: string) {
    ...
    if (current) updateRun({ ...current, state: "stopping" });
    try {
  ...
  async function dismissInactiveRuns() {
    ...
    removedRunIds.forEach((runId) => pendingLogsByRunId.delete(runId));
    runsById.value = Object.fromEntries(
      Object.entries(runsById.value).filter(
        ([runId]) => !removedRunIds.has(runId),
      ),
    );
    logsByRunId.value = Object.fromEntries(
  ...
  async function openInFileManager(directory: string) {
    await invoke("open_in_file_manager", { directory });
  }
  async function openWithSource(sourceId: string, directory: string) {
    await invoke("open_with_source", { sourceId, directory });
  }
  async function saveOpenSource(source: OpenSourceConfig) {
    const saved = await invoke<OpenSourceConfig>("save_open_source", {
      source,
    });
    await refreshOpenSources();
    return saved;
  }
  async function deleteOpenSource(sourceId: string) {
    await invoke("delete_open_source", { sourceId });
    await refreshOpenSources();
  }
  async function setAutostart(enabled: boolean) {
    autostartEnabled.value = await invoke<boolean>("set_autostart_enabled", {
      enabled,
    });
  }
  ...
  function formatUptime(startedAt: number | null | undefined) {
    ...
    const totalSeconds = Math.max(
      0,
      Math.floor((now.value - startedAt) / 1_000),
    );
    return [
